/*
  ...
  ### Output:
  <svg fill="currentColor" aria-hidden="true" viewBox="0 0 24 24"><path d="M7.52 21.48C4.25 19.94 ..."/></svg>
*/

const fs = require("fs");
const path = require("path");
const sourcePath = path.join(__dirname, "../originals/");
...
fs.readdir(sourcePath, function(error, files) {
  ...
  files.forEach(function(fileName) {
    ...
    const processedFile = file.replace(
      `width="24" height="24"`,
      `focusable="false" aria-hidden="true" width="16" height="16" fill="currentColor"`
    );
